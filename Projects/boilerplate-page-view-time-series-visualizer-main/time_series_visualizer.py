...
def draw_line_plot():
    ...
    ax.plot(df.index, df['value'], linewidth=1, color='r')
...
def draw_bar_plot():
    ...
    df_bar = df_bar.groupby(["year", "month"])["value"].mean().unstack('month').reindex(columns=range(1, 13))

    # Draw bar plot
    ax = df_bar.plot(kind='bar', figsize=(10,8), legend=True)
    fig = ax.figure
    ax.set_xlabel("Years")
